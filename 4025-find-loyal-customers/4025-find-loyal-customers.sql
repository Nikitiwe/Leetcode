...
SELECT customer_id #, ROUND(1.0*SUM(IF(action = 'confirmed', 1, 0))/COUNT(*), 2) AS confirmation_rate
FROM customer_transactions AS t1 #LEFT JOIN Confirmations AS t2 USING(user_id)
GROUP BY 1
