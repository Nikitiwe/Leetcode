...
public:
    bool isPossible(int n, vector<vector<int>>& s) {
        ...
        if (id.size()==4)
        {
            long a=id[0]*x+id[1], b=id[2]*x+id[3];
            if (m.count(a)==0&&m.count(b)==0) return 1;
            else
            {
                long c=id[0]*x+id[2], d=id[1]*x+id[3];
                if (m.count(c)==0&&m.count(d)==0) return 1;
                else
                {
                    long e=id[0]*x+id[3], f=id[1]*x+id[2];
        ...
        if (id.size()==2)
        {
            ...
            else
            {
                ...
                {
                    long a=i*x+id[0], b=i*x+id[1];
                    if (m.count(a)==0&&m.count(b)==0) return 1;
                }
                for (int i=id[0]+1; i<id[1]; i++)
                {
                    long a=id[0]*x+i, b=i*x+id[1];
                    if (m.count(a)==0&&m.count(b)==0) return 1;
                }
                for (int i=id[1]+1; i<=n; i++)
                {
                    long a=id[0]*x+i, b=id[1]*x+i;
                    if (m.count(a)==0&&m.count(b)==0) return 1;
                }
            }
        }
